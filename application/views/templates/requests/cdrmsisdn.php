        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                <div id="headerdiv" class="box-header with-border">
                    <h3 class="box-title">Request CDR Against Mobile Number </h3>
...
<script>
    ...
    $(document).ready(function () {        
        ...
        //to check duration permissions
        check_duration_permissions();
...
//                    required: "Enter date from"
    ...
    function backdate(value) {
        ...
        return datefrommonthago;
             ...
             function submitrequestform(){
                 ...
                 // optional "Requested Attachment" file input picked up on
                 // this form actually rides the POST. serializeArray() only
                 // walks plain inputs and silently drops <input type="file">.
                 // The accompanying processData/contentType:false on the
                 // ajax call below preserves the multipart boundary jQuery
                 // would otherwise rewrite into application/x-www-form-urlencoded.
    ...
    //to check duration permissions
    function check_duration_permissions() {
    
        duration_permission_global=0;
        var request_permission=0;
        var requesttype = $("#ChooseTemplate").val();                
        var msisdn = $("#inputmsisdn").val();
         var startdate = $("#startDate").val();
        var enddate = $("#endDate").val();
        var result = {requesttype: requesttype, msisdn: msisdn,startdate:startdate,enddate:enddate}
            ...
            success: function (result) {
                if (result == 2)
                {
                    ...
                    var result = JSON.parse(result);
                     request_permission = result.permission;
...
//                            $("#startdate_duration").val(request_startdate);
                            ...
                            $("#notification_msgbasic1").addClass('alert-success');
                            $('#dataform').show();
                            break;
                        case 1:
                            $("#notification_msg_divbasic1").html(request_message);
         ...
         duration_permission_global = request_permission
        ...
        if (cdr_startdate != '' && cdr_enddate != '') {
            ...
            if ((toTimestamp(startdate) > toTimestamp(cdr_enddate)) && (toTimestamp(enddate) > toTimestamp(cdr_enddate))) {
                ...
                var send_perm=null;        
                        ...
                        send_perm=check_duration_permissions(); 
                setTimeout(function () {   
                    if(duration_permission_global != 2 ){
                   duration_validate=true; 
                }
                 },1000);
