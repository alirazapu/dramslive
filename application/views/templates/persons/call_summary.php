 ...
 */
$otherphone = '';
if (!empty($search_post['otherphone'])) {
    $counter = 1;
    foreach ($search_post['otherphone'] as $o_phone) {
        if (sizeof($search_post['otherphone']) == $counter)
            $otherphone .= $o_phone;
        else
            $otherphone .= $o_phone . ',';
...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Cell Log Summary</li>
    </ol>
</section>
<!-- Main content -->
    ...
    <div class="container-fluid">
        <div class="">
            <div class="box box-primary">
                ...
                <form role="form" id="call_summary_form" name="search_form" method="post"  action="<?php echo URL::site('persons/call_summary/?id=' . $_GET['id']); ?>" >
                        ...
                        <div class="box-body" style="<?php echo (!empty($search_post)) ? 'display:block;' : ''; ?>">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control " name="phone_number" id="phone_number" onchange="person_bparty()">                                                        
                                        ...
                                        <?php
                                        try {
                                            $sims_list = Helpers_Person::get_person_inuse_SIMs($personid);
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" id="otherphonenumbers" value="<?php echo $otherphone ?>" />
            ...
            <div class="col-xs-12">
                ...
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title"><i class="fa fa-mobile-phone"></i> Call Summary of <?php echo Helpers_Person::get_person_name($personid); ?> </h3>
                    ...
                    <div class="box-body">
                        <div class="table-responsive">
                            <table id="callsummary" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        <th class="no-sort">Phone Number</th>
                                        <th class="no-sort">B Party</th>
                                        <th >Incoming Call</th>
                                        <th >Outgoing Call</th>
                                        <th >Grand Total</th>
                                        <th class="no-sort">Other Detail</th>
                                    </tr>
                                </thead>
                                <tbody>
...
// draw the 'current' page
        ...
        objDT = $('#callsummary').dataTable(
                {"aaSorting": [[4, "desc"]],
                    "bPaginate": true,
                    "bProcessing": true,
                    //"bStateSave": true,
                    "bServerSide": true,
                    "sAjaxSource": "<?php echo URL::site('persons/ajaxcallsummary/?id=' . $_GET['id'], TRUE); ?>",
    ...
    function person_bparty() {
        var otherphonenumbers = $('#otherphonenumbers').val();
        var phonenumber = $('#phone_number').val();
        var searchresults = {phone: phonenumber, otherphonenumbers: otherphonenumbers}
        $.ajax({
    ...
    $("#call_summary_form").validate({
        ...
        messages: {
            phone_number: {
                check_list: "Please select person Phone Number",
            },
            key: {
    ...
    function clearSearch() {
        window.location.href = '<?php echo URL::site('persons/call_summary/?id=' . $_GET['id'], TRUE); ?>';
    }
    function excel(id) {
        $('#xport').val('excel');
