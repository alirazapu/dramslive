                      ...
                      enctype="multipart/form-data">
                        ...
                        <div class="col-sm-6" >
                            <div class="form-group">
                                <label class="control-label">CNIC(s) No.<span class="text-danger">(*)</span>(Entered multiple CNIC's in (,) separatd form.)</label>
...
<script type="text/javascript">
    ...
    function submitrequestform() {
        ...
        if ($('#userrequest').valid()) {
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo URL::site('adminrequest/admin_nadra_bulk_send') ?>",
                data: data,
                cache: false,
                contentType: false,
                processData: false,
                success: function (result) {
                    ...
                    if (result == 1) {
                        swal({
                            ...
                            function (isConfirm) {
                                if (isConfirm) {
                                    window.location.href = '<?php echo URL::site('adminrequest/nadra_bulk_request_sent_form', TRUE); ?>';
                                } else {
    ...
    $(document).ready(function () {
        ...
        objDT1 = $('#requeststatus').dataTable(
            {"aaSorting": [[5, "desc"]],
                ...
                //"bStateSave": true,
                "bServerSide": true,
                "sAjaxSource": "<?php echo URL::site('adminrequest/ajax_nadra_request_status', TRUE); ?>",
...
// alert('test');
    ...
    function  UpdateRequestStatusDetails() {
        ...
        $.ajax({
            url: "<?php echo URL::site("adminrequest/update_nadra_request_status"); ?>",
            type: 'POST',
            data: result,
            cache: false,
            success: function (result) {
