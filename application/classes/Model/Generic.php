...
//        $phone_number_exist = DB::query(Database::SELECT, $sql)->execute()->current();
//        // to check person existed       
//       // $chk = Helpers_Utilities::get_person_id_with_foreign_identity($cnic_number_original);
//        $chk = Helpers_Utilities::get_person_id_with_cnic($cnic_number_original);
        //if person not exist then new entry 
        // if ($chk == 0) {
//            $query = DB::insert('person', array('first_name', 'last_name', 'cnic_number', 'address', 'user_id','is_foreigner'))
//                    ->values(array($data['person_name'], $data['person_name1'], $cnic_without_chr, $data['address'], $data['user_id'], $data['is_foreigner']))
//                    ->execute();
//            $person_id = $query[0];
//            /*
//             * End Update Query
...
//                /*
//                 * Update Foregin Person Table
//                 */
//                DB::update('person_foreigners_profile')->set(array('person_id' => $person_id))
//                        ->where('cnic_number', '=', $cnic_number_original)
//                        ->execute();
//            } else {
//                $query = DB::insert('person_nadra_profile', array('person_id', 'cnic_number', 'user_id'))
//                        ->values(array($person_id, $cnic_without_chr, $data['user_id']))
//                        ->execute();
//            }
//            $query = DB::insert('person_category', array('person_id', 'category_id', 'project_id', 'reason', 'user_id', 'added_on'))
//                    ->values(array($person_id, 0,$data['inputproject'][0],'', $data['user_id'], $current_date))
//                    ->execute();
//            //person identity update            
//            $identity_data['idnno'] = $cnic_number_original;
//            $identity_data['idnname']=$person_identity_type;
//            $update_identity = Model_Personprofile::update_identity($identity_data, '', $person_id);
//           
//            if (!empty($phone_number_exist)) {
//                // if phone number exist then first checking sim user is existed or not
//                $chksimuser= Helpers_Utilities::check_sim_user_exist($data['mobile_number']);
//                // if sim user not exist
//                if($chksimuser==0 || $chksimuser==''){
//                   // print_r('new person person id'); exit;
//                     DB::update('person_phone_number')->set(array('sim_owner' => $person_id,'person_id' => $person_id, 'imsi_number' => $data['imsi'], 'sim_activated_at' => $newDate, 'status' => $data['StatusRadios'], 'connection_type' => $data['ConnectionTypeRadios'], 'mnc' => $data['company_name_get'], 'user_id' => $data['user_id']))
//                        ->where('phone_number', '=', $data['mobile_number'])
//                        ->execute();
//                     
//                }else{
//                    // if sim user not existed
//                  //  print_r('new person sim owenr'); exit;
//                     DB::update('person_phone_number')->set(array('sim_owner' => $person_id, 'imsi_number' => $data['imsi'], 'sim_activated_at' => $newDate, 'status' => $data['StatusRadios'], 'connection_type' => $data['ConnectionTypeRadios'], 'mnc' => $data['company_name_get'], 'user_id' => $data['user_id']))
//                        ->where('phone_number', '=', $data['mobile_number'])
//                        ->execute();
//                }
//                // if phone number not existed then new entry
//            } else {
...
//                        ->values(array($person_id, $person_id, $data['mobile_number'], $data['imsi'], $newDate, $data['StatusRadios'], $data['ConnectionTypeRadios'], $data['company_name_get'], $data['user_id']))
//                        ->execute();
//                
//            }
//            if(!empty($data['imei'])){
//                // to check if imei number exist
//            $chkimei = Helpers_Utilities::check_imei_exist($data['imei']);
//            // if imei number not exist then new entry 
//            if ($chkimei == 0 AND $data['imei'] > 0) {
//                $query = DB::insert('person_phone_device', array('person_id', 'imei_number', 'phone_name', 'in_use_since', 'last_interaction_at', 'user_id'))
//                        ->values(array($person_id, $data['imei'], $data['phone_name'], $newDate, $newDate, $data['user_id']))
//                        ->execute();
//                $device_id = $query[0];
//                DB::update('person_device_numbers')->set(array('is_active' => 0))
//                        ->where('phone_number', '=', $data['mobile_number'])
//                        ->execute();
//                $query = DB::insert('person_device_numbers', array('device_id', 'phone_number', 'is_active', 'first_use'))
//                        ->values(array($device_id, $data['mobile_number'], 1, $newDate))
//                        ->execute();
//                
//            } elseif ($chkimei != 0 AND $data['imei'] > 0) {
//                $getdeviceid = Helpers_Utilities::get_device_id($data['imei']);
//                DB::update('person_phone_device')->set(array('person_id' => $person_id, 'user_id' => $data['user_id']))
//                        ->where('id', '=', $getdeviceid)
//                        ->execute();
//                $chkdeviceno = Helpers_Utilities::check_device_number_exist($getdeviceid, $data['mobile_number']);
//                if ($chkdeviceno == 0) {
//                    DB::update('person_device_numbers')->set(array('is_active' => 0))
//                            ->where('phone_number', '=', $data['mobile_number'])
//                            ->execute();
//                    $query = DB::insert('person_device_numbers', array('device_id', 'phone_number', 'is_active', 'first_use'))
//                            ->values(array($getdeviceid, $data['mobile_number'], 1, $newDate))
//                            ->execute();
//                } else {
//                    DB::update('person_device_numbers')->set(array('is_active' => 0))
//                            ->where('phone_number', '=', $data['mobile_number'])
...
//                            ->where('phone_number', '=', $data['mobile_number'])
//                            ->and_where('device_id', '=', $getdeviceid)
//                            ->execute();
//                }
//            }
//        }
...
//            }
//            $uid = $login_id;
//            Helpers_Profile::user_activity_log($uid, 22, NULL, NULL, $person_id);
//        } else {
//            $person_id = $chk;
//            DB::update('person')->set(array('first_name' => $data['person_name'], 'last_name' => $data['person_name1'], 'address' => $data['address'],'cnic_number' => $cnic_without_chr, 'user_id' => $data['user_id'], 'is_foreigner' => $data['is_foreigner']))
//                    ->where('person_id', '=', $person_id)
//                    ->execute();
//            // to update person phone number table
//            if (!empty($phone_number_exist)) {
//                $chksimuser= Helpers_Utilities::check_sim_user_exist($data['mobile_number']);
//                if($chksimuser==0 || $chksimuser==''){
//                   // print_r('existing person person id'); exit;                  exit();
//                     DB::update('person_phone_number')->set(array('sim_owner' => $person_id,'person_id' => $person_id, 'imsi_number' => $data['imsi'], 'sim_activated_at' => $newDate, 'status' => $data['StatusRadios'], 'connection_type' => $data['ConnectionTypeRadios'], 'mnc' => $data['company_name_get'], 'user_id' => $data['user_id']))
//                        ->where('phone_number', '=', $data['mobile_number'])
//                        ->execute();
//                }else{
//                   // print_r('existing sim owner'); exit;
//                     DB::update('person_phone_number')->set(array('sim_owner' => $person_id, 'imsi_number' => $data['imsi'], 'sim_activated_at' => $newDate, 'status' => $data['StatusRadios'], 'connection_type' => $data['ConnectionTypeRadios'], 'mnc' => $data['company_name_get'], 'user_id' => $data['user_id']))
//                        ->where('phone_number', '=', $data['mobile_number'])
//                        ->execute();
//                }
...
//            }
//            if(!empty($data['imei'])){
//            // to check device existed
//            $chkimei = Helpers_Utilities::check_imei_exist($data['imei']);
//            if ($chkimei == 0 AND $data['imei'] > 0) {
//                $query = DB::insert('person_phone_device', array('person_id', 'imei_number', 'phone_name', 'in_use_since', 'last_interaction_at', 'user_id'))
//                        ->values(array($chk, $data['imei'], $data['phone_name'], $newDate, $newDate, $data['user_id']))
//                        ->execute();
//                $device_id = $query[0];
//                // to check device number existed
//                $chkdeviceno = Helpers_Utilities::check_device_number_exist($device_id, $data['mobile_number']);
//                if ($chkdeviceno == 0) {
//                    DB::update('person_device_numbers')->set(array('is_active' => 0))
//                            ->where('phone_number', '=', $data['mobile_number'])
//                            ->execute();
...
//                $getdeviceid = Helpers_Utilities::get_device_id($data['imei']);
//                DB::update('person_phone_device')->set(array('person_id' => $person_id, 'user_id' => $data['user_id']))
//                        ->where('imei_number', '=', $data['imei'])
//                        ->execute();
//                $chkdeviceno = Helpers_Utilities::check_device_number_exist($getdeviceid, $data['mobile_number']);
//                if ($chkdeviceno == 0) {
//                    DB::update('person_device_numbers')->set(array('is_active' => 0))
//                            ->where('device_id', '=', $getdeviceid)
//                            ->execute();
//                    $query = DB::insert('person_device_numbers', array('device_id', 'phone_number', 'is_active', 'first_use'))
//                            ->values(array($getdeviceid, $data['mobile_number'], 1, $newDate))
//                            ->execute();
...
//            }
//            $uid = $login_id;
//            Helpers_Profile::user_activity_log($uid, 23, NULL, NULL, $person_id);
//        }
//        // $current_date  $user_obj->id;  
//        $sql = "SELECT * FROM person_category where person_id = {$person_id}";        
//        $person_cat = DB::query(Database::SELECT, $sql)->execute()->current();
//        if(empty($person_cat))
//        {
//            $query = DB::insert('person_category', array('person_id','category_id', 'user_id', 'added_on'))                
//                            ->values(array($person_id,0, $user_obj->id, $current_date))                                
//                            ->execute(); 
//        }   
