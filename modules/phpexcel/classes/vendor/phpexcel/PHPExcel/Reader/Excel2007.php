        ...
        foreach ($rels->Relationship as $rel) {
            switch ($rel["Type"]) {
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    //print_r($zip);
                    //print_r($dir);
                    $xpath['Target']= !empty($xpath['Target'])?$xpath['Target']:''; 
                   // print_r("$dir/$xpath[Target]");
                   // exit;
...
//                                                echo 'Default', PHP_EOL;
    ...
    //                                    echo 'Value is ', $value, PHP_EOL;
                   ...
                   /* if ((!$this->readDataOnly) || (!empty($this->loadSheetsOnly))) {
                    ...
                    if ((!$this->readDataOnly) || (!empty($this->loadSheetsOnly))) {
    // Safe check for activeTab
    $activeTab = 0; // default

    if (isset($xmlWorkbook->bookViews->workbookView)) {
        $workbookView = $xmlWorkbook->bookViews->workbookView;

        if (isset($workbookView["activeTab"])) {
            $activeTab = intval($workbookView["activeTab"]);
        }
    }

    // keep active sheet index if sheet is still loaded, else set first sheet as active
    if (isset($mapSheetId[$activeTab]) && $mapSheetId[$activeTab] !== null) {
        $excel->setActiveSheetIndex($mapSheetId[$activeTab]);
    } else {
        if ($excel->getSheetCount() == 0) {
